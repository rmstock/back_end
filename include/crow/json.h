                ...
                switch(c)
                {
                    ...
                    default:
                        if (0 <= c && c < 0x20)
                        {
                            ...
                            ret += static_cast<char>(to_hex(c/16));
                            ret += static_cast<char>(to_hex(c%16));
                        }
                        else
                            ret += c;
                                ...
                                case 'u':
                                    {
                                        ...
                                        if (code >= 0x800)
                                        {
                                            *tail++ = static_cast<char>(0xE0 | (code >> 12));
                                            *tail++ = static_cast<char>(0x80 | ((code >> 6) & 0x3F));
                                            *tail++ = static_cast<char>(0x80 | (code & 0x3F));
                                        }
                                        else if (code >= 0x80)
                                        {
                                            *tail++ = static_cast<char>(0xC0 | (code >> 6));
                                            *tail++ = static_cast<char>(0x80 | (code & 0x3F));
                                        }
                                        else
                                        {
                                            *tail++ = static_cast<char>(code);
                                        }
                                        head += 4;
                                    }
                                    break;
            ...
            void emplace_back(rvalue&& v)
            {
                if (!lremain_)
                {
                    ...
                    for(auto& x : *this)
                        *p2++ = std::move(x);
                    l_.reset(p);
                    lremain_ = static_cast<uint16_t>(new_size - lsize_);
                }
