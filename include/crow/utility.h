...
#undef CROW_INTERNAL_PARAMETER_TAG
        ...
        static inline bool is_parameter_tag_compatible(uint64_t a, uint64_t b)
        {
            ...
            if (b == 0)
                return a == 0;
            int sa = static_cast<int>(a%6);
            int sb = static_cast<int>(a%6);
            if (sa == 5) sa = 4;
            if (sb == 5) sb = 4;
            if (sa != sb)
                return false;
...
template <typename F, typename Set>
        ...
        struct arguments
        {
            ...
            using type = 
                typename subarguments::template push<typename single_tag_to_type<static_cast<int>(Tag%6)>::type>;
        };
        ...
        {
            template <template <typename ... Args> class U>
            using rebind = typename pop_back_helper<typename gen_seq<static_cast<unsigned int>(sizeof...(T)-1)>::type, std::tuple<T...>>::template rebind<U>;
        };
