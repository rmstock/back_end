...
namespace crow
{
    ...
    struct RuleParameterTraits
    {
        ...
        {
            (static_cast<self_t*>(this))->methods_ = 1 << (int)method;
            return static_cast<self_t&>(*this);
        }
...
public:
        ...
        std::pair<unsigned, routing_params> find(const std::string& req_url, const Node* node = nullptr, unsigned pos = 0, routing_params* params = nullptr) const
        {
            ...
            if (node->param_childrens[static_cast<int>(ParamType::INT)])
            {
                ...
                if ((c >= '0' && c <= '9') || c == '+' || c == '-')
                {
                    ...
                    if (errno != ERANGE && eptr != req_url.data()+pos)
                    {
                        ...
                        auto ret = find(req_url, &nodes_[node->param_childrens[static_cast<int>(ParamType::INT)]], static_cast<unsigned int>(eptr - req_url.data()), params);
                        update_found(ret);
            ...
            if (node->param_childrens[static_cast<int>(ParamType::UINT)])
            {
                ...
                if ((c >= '0' && c <= '9') || c == '+')
                {
                    ...
                    if (errno != ERANGE && eptr != req_url.data()+pos)
                    {
                        ...
                        auto ret = find(req_url, &nodes_[node->param_childrens[(int)ParamType::UINT]], (unsigned int)(eptr - req_url.data()), params);
                        update_found(ret);
            ...
            if (node->param_childrens[(int)ParamType::DOUBLE])
            {
                ...
                if ((c >= '0' && c <= '9') || c == '+' || c == '-' || c == '.')
                {
                    ...
                    if (errno != ERANGE && eptr != req_url.data()+pos)
                    {
                        ...
                        auto ret = find(req_url, &nodes_[node->param_childrens[(int)ParamType::DOUBLE]], (unsigned int)(eptr - req_url.data()), params);
                        update_found(ret);
            ...
            if (node->param_childrens[(int)ParamType::STRING])
            {
                ...
                if (epos != pos)
                {
                    ...
                    auto ret = find(req_url, &nodes_[node->param_childrens[(int)ParamType::STRING]], (unsigned int)epos, params);
                    update_found(ret);
            ...
            if (node->param_childrens[(int)ParamType::PATH])
            {
                ...
                {
                    params->string_params.push_back(req_url.substr(pos, epos-pos));
                    auto ret = find(req_url, &nodes_[node->param_childrens[(int)ParamType::PATH]], (unsigned int)epos, params);
                    update_found(ret);
                    params->string_params.pop_back();
                }
            }
            ...
            for(auto& kv : node->children)
            {
                ...
                {
                    auto ret = find(req_url, child, (unsigned int)(pos + fragment.size()), params);
                    update_found(ret);
                }
            }
    ...
    private:
        ...
        unsigned new_node()
        {
            ...
            return (unsigned int)(nodes_.size() - 1);
        }
            ...
            ruleObject->foreach_method([&](int method)
                    {
                        ...
                        per_methods_[method].trie.add(rule, (unsigned int)(per_methods_[method].rules.size() - 1));
