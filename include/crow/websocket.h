        ...
        class Connection : public connection
        {
			public:
					...
					, accept_handler_(std::move(accept_handler))
				{
                    ...
                    uint8_t digest[20];
                    s.getDigestBytes(const_cast<unsigned char*>(digest));   
                ...
                void send_pong(const std::string& msg)
                {
                    dispatch([this, msg]{
                        char buf[3] = "\x8A\x00";
                        buf[1] = static_cast<char>(buf[1] + msg.size());
            ...
            protected:
                ...
                std::string build_header(int opcode, size_t size)
                {
                    ...
                    buf[0] = static_cast<char>(buf[0] + opcode);
                    if (size < 126)
                    {
                        buf[1] = static_cast<char>(buf[1] + size);
                        return {buf, buf+2};
                    }
                    else if (size < 0x10000)
                    {
                        buf[1] = static_cast<char>(buf[1] + 126);
                        *reinterpret_cast<uint16_t*>(buf+2) = htons(static_cast<__uint16_t>(size));
                        return {buf, buf+4};
                    }
                    else
                    {
                        buf[1] = static_cast<char>(buf[1] + 127);
                        *reinterpret_cast<uint64_t*>(buf+2) = ((1==htonl(1)) ? static_cast<uint64_t>(size) : (static_cast<uint64_t>(htonl((size) & 0xFFFFFFFF)) << 32) | htonl(static_cast<__uint32_t>((size) >> 32)));
                        return {buf, buf+10};
                    }
                }
                ...
                void do_read()
                {
                    ...
                    switch(state_)
                    {
                        ...
                        case WebSocketReadState::Len64:
                            {
                                boost::asio::async_read(adaptor_.socket(), boost::asio::buffer(&remaining_length_, 8), 
                                    ...
                                    {
                                        is_reading = false;
                                        remaining_length_ = ((1==ntohl(1)) ? (remaining_length_) : (static_cast<uint64_t>(ntohl((remaining_length_) & 0xFFFFFFFF)) << 32) | ntohl(static_cast<__uint32_t>((remaining_length_) >> 32)));
#ifdef CROW_ENABLE_DEBUG
                        ...
                        case WebSocketReadState::Mask:
                                boost::asio::async_read(adaptor_.socket(), boost::asio::buffer(reinterpret_cast<char*>(&mask_), 4), 
                                    [this](const boost::system::error_code& ec, std::size_t 
#ifdef CROW_ENABLE_DEBUG 
                                        bytes_transferred
#endif
                ...
                void handle_fragment()
                {
                    ...
                    {
                        fragment_[i] ^= (reinterpret_cast<char*>(&mask_))[i%4];
                    }
                    switch(opcode())
                    {
                        case 0: // Continuation
